<template>
  ...
  <FreeSWITCHCallUI ref="freeswitch" />
  <Dialog
    v-model="show"
    :options="{
      title: __('Make Call'),
    ...
    <template #body-content>
      <div class="flex flex-col gap-4">
        ...
        <FormControl
          ...
          :options="['Twilio', 'Exotel', 'FreeSWITCH']"
        />
...
import FreeSWITCHCallUI from '@/components/Telephony/FreeSWITCHCallUI.vue'
...
const enabledIntegrations = computed(() =>
  [
    ...
    { key: 'freeswitch', label: 'FreeSWITCH', ref: freeswitch },
...
function makeCallUsing() {
  ...
  if (callMedium.value === 'FreeSWITCH') {
    freeswitch.value.makeOutgoingCall(mobileNumber.value)
  }
  show.value = false
}
