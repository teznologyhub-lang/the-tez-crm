  ...
  <div class="flex h-full flex-col gap-6 px-6 py-8">
    ...
    <div v-if="telephonyAgent.doc" class="flex-1 flex flex-col overflow-y-auto">
        ...
        <div class="flex items-center gap-1">
          <FormControl
            ...
            :options="[
              ...
              { label: __('FreeSWITCH'), value: 'FreeSWITCH' },
            ]"
      ...
      >
        <div class="flex flex-col">
          <div class="text-p-base font-medium text-ink-gray-7 truncate">
            {{ __('FreeSWITCH Extension') }}
          </div>
          <div class="text-p-sm text-ink-gray-5">
            {{ __('Set your FreeSWITCH extension used to make and receive calls.') }}
          </div>
        </div>
        <div>
          <FormControl
            v-model="telephonyAgent.doc.freeswitch_extension"
            class="flex-1 truncate w-44 p-1"
      ...
      >
        <div class="flex flex-col">
          <div class="text-p-base font-medium text-ink-gray-7 truncate">
            {{ __('FreeSWITCH Password') }}
          </div>
          <div class="text-p-sm text-ink-gray-5">
            {{ __('Set your FreeSWITCH SIP extension password.') }}
          </div>
        </div>
        <div>
          <FormControl
            v-model="telephonyAgent.doc.freeswitch_password"
            type="password"
            class="flex-1 truncate w-44 p-1"
      ...
      >
        <div class="flex flex-col gap-1">
          ...
          <span class="text-p-sm text-ink-gray-6">
            {{
              __('Configure your FreeSWITCH Telephony Integration Settings here')
            }}
          </span>
        </div>
        <Button
          :label="
            isEnabled('freeswitch') ? __('Update Configuration') : __('Configure')
          "
          @click="emit('updateStep', 'freeswitch-settings')"
        />
      </div>
    </div>
    <ErrorMessage
